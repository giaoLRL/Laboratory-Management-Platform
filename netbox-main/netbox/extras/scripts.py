    ...
    def validate(self, value):
        ...
        if value:
            try:
                ...
                raise forms.ValidationError(_('Enter a valid IP network.'))

# IPAddressFormField stub (ipam module removed)
    ...
    def validate(self, value):
        ...
        if value:
            try:
                ...
                raise forms.ValidationError(_('Enter a valid IP address.'))

def prefix_validator(value):
    try:
        IPNetwork(value)
    except AddrFormatError:
        raise forms.ValidationError(_('%(value)s is not a valid IP prefix.') % {'value': value})

def MaxPrefixLengthValidator(limit):
    def validate(value):
        net = IPNetwork(value)
        if net.prefixlen > limit:
            raise forms.ValidationError(_('Prefix length must not exceed %(limit)s.') % {'limit': limit})
    return [validate]

def MinPrefixLengthValidator(limit):
    def validate(value):
        net = IPNetwork(value)
        if net.prefixlen < limit:
            raise forms.ValidationError(_('Prefix length must be at least %(limit)s.') % {'limit': limit})
    return [validate]
